...
def ids(nodo, profundidad_maxima):
    ...
    for profundidad in range(profundidad_maxima):
        solucion, visitados = dfs_limitado(nodo, profundidad)
        nodos_visitados += visitados

        if solucion:
            return solucion, nodos_visitados
    return None, nodos_visitados


def dfs_limitado(estado_inicial, estado_objetivo, limite):
    frontera = ([estado_inicial, 0])
    visitados = set()
    nodos_expandidos = 0
    ...
    while frontera:
        ...
        if nodo_actual.es_solucion(estado_objetivo):
            return nodo_actual, visitados, nodos_expandidos
...
if __name__ == "__main__":
    ...
    estado_objetivo = Nodo(tablero_objetivo)
    solucion, visitados = ids(
        estado_inicial, estado_objetivo, profundidad_maxima)
